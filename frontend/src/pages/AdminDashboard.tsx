  ...
  if (user?.role !== 'admin' && user?.role !== 'instructor') {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center">
        ...
        <p className="text-slate-400 mt-2">You do not have permission to view the instructor dashboard.</p>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass rounded-3xl p-6 bg-[#151518] flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-4xl font-black text-white">{myCourses.length}</p>
          </div>
        </div>
        <div className="glass rounded-3xl p-6 bg-[#151518] flex items-center gap-6">
      ...
      {/* Course List */}
      <div>
        <h2 className="text-2xl font-bold text-white tracking-tight mb-6">Manage Your Courses</h2>

        {isLoading ? (
          <div className="space-y-4">
            {[1, 2, 3].map(i => <div key={i} className="glass h-24 rounded-2xl bg-[#151518]/40 animate-pulse" />)}
          </div>
        ) : myCourses.length > 0 ? (
          <div className="space-y-4">
            {myCourses.map((course: any) => (
              <div key={course.id} className="glass rounded-2xl p-6 bg-[#151518] border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 hover:shadow-xl hover:border-teal-200 transition-all group">
                <div className="flex items-center gap-6 w-full md:w-auto">
                  <div className="w-32 h-20 bg-white/5 rounded-xl overflow-hidden shrink-0 relative flex items-center justify-center">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-xl text-white group-hover:text-teal-600 transition-colors leading-tight">
                      {course.title}
                    </h3>
                    <div className="mt-2 flex items-center gap-4 text-sm font-medium text-slate-400">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-none border-slate-100">
                  <Link
                    ...
                    className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-[#0a0a0b] text-slate-300 font-bold rounded-xl hover:bg-white/5 hover:text-white transition-colors border border-white/10"
                  >
          ...
          <div className="glass rounded-3xl p-16 text-center border-dashed border-2 border-white/10 bg-[#151518]/50 flex flex-col items-center">
            <div className="w-20 h-20 bg-[#0a0a0b] rounded-full flex items-center justify-center text-slate-400 mb-4 shadow-sm">
              <BookOpen className="w-10 h-10" />
            </div>
            <h3 className="text-2xl font-bold text-white mb-2">You haven't created any courses yet</h3>
            <p className="text-slate-400 max-w-md mx-auto mb-8">Start sharing your knowledge with the world by building your first interactive course.</p>
            <Link
              to="/admin/courses/create"
