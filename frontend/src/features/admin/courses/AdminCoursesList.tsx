...
export default function AdminCoursesList() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12 pt-4">
      <div className="glass rounded-[2rem] p-8 sm:p-10 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between text-left">
        ...
        <div className="relative z-10 w-full max-w-3xl">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight">
            Courses <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-violet-600">Management</span>
          </h1>
          <p className="mt-4 text-lg text-slate-600 font-medium">Control panel for all your educational content.</p>
        </div>
        <div className="relative z-10 mt-6 md:mt-0 flex shrink-0">
          <Link 
            to="/admin/courses/create"
            className="group flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:-translate-y-1 transition-all"
          >
      ...
      <div className="glass rounded-[2rem] p-8 bg-white overflow-hidden shadow-sm">
        {isLoading ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="py-4 font-bold text-slate-500 uppercase tracking-wider text-xs">Course Title</th>
                  <th className="py-4 font-bold text-slate-500 uppercase tracking-wider text-xs">Instructor</th>
                  <th className="py-4 font-bold text-slate-500 uppercase tracking-wider text-xs">Category</th>
                ...
                {myCourses.map((course: any) => (
                  <tr key={course.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 font-medium">
                      <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-bold">{course.category || 'General'}</span>
                    </td>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 mt-2">Create your first course to populate the list.</p>
          </div>
        )}
      </div>
    </div>
