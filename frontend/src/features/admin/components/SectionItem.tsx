...
export default function SectionItem({ section, index, onAddLecture }: any) {
  return (
    <div className="glass rounded-[1.5rem] bg-white border border-slate-200 overflow-hidden shadow-sm group">
      {/* Section Header */}
      <div className="bg-slate-50 border-b border-slate-100 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 hover:text-teal-600 hover:border-teal-200 rounded-xl font-bold text-sm transition-all shadow-sm"
          >
      ...
      <div className="p-4 space-y-3">
        {section.lessons && section.lessons.length > 0 ? (
          ...
          <div className="p-8 text-center border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/50">
            <p className="text-slate-500 font-medium text-sm">No lectures created yet. Click "Add Lecture" to upload a video.</p>
          </div>
        )}
      </div>
    </div>
